...
function displayMovis() {
  ...
  for (let i = 0; i < moviesArr.length; i++) {
    ...
    <div class="cart-img">
      ...
      <button class="btn btn-dark fw-bold rounded-3 book-ticket">Book Ticket</button>
    </div>`;

    element.append(creatElem);
  }
}
    ...
    bookTicket[i].addEventListener("click", function () {
      for (let j = 0; j < moviesArr.length; j++) {
        if (i === j) {
          ...
          movies.style.filter = "blur(10px)";
          document.body.style.overflow = "hidden";
          modalContent.innerHTML = `
            
            <h2 class="fw-bold mb-3">${title} (${year})</h2>
            <h5 class="tw-bold"><strong>Price</strong> ${price}</h5>
            <h5 class="tw-bold"><strong>Vat</strong> ${vat} ৳</h5>
            <h2 class="tw-bold mb-3"><strong>Subtotal</strong> ${subtotal} ৳</h2>
            <button class="btn btn-danger fw-bold rounded-3 buy-now">Buy Now</a>
          `;
...
// const valCal = (Number(price) * 7) / 100;
// const subtotal = Number(price) + valCal;
// console.log(moviesArr[j]);
// modalWindow.innerHTML = `
//   <div class="clos position-absolute">❌</div>
//   <h2 class="fw-bold mb-3">${moviesArr[j].title} (${moviesArr[j].year})</h2>
//   <h5 class="tw-bold"><strong>Price</strong> ${moviesArr[j].ticketPrice}</h5>
//   <h5 class="tw-bold"><strong>Vat</strong> ${valCal} ৳</h5>
//   <h2 class="tw-bold"><strong>Subtotal</strong> ${subtotal} ৳</h2>
//   <a href="#" class="btn btn-danger fw-bold rounded-3">Buy Now</a>
// `;
